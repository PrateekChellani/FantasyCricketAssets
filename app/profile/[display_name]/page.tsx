...
function normalizeXLink(v: string | null | undefined): { handle: string | null; url: string | null } {
  ...
  if (raw.startsWith('http://') || raw.startsWith('https://')) {
    try {
      const u = new URL(raw);
      const h = u.pathname.replace(/\//g, '').replace('@', '').trim() || null;
      return { handle: h ? `@${h}` : raw, url: raw };
    } catch {
      return { handle: raw, url: raw };
    }
  }

  const h = raw.replace(/^@/, '').trim();
  if (!h) return { handle: null, url: null };
  return { handle: `@${h}`, url: `https://x.com/${encodeURIComponent(h)}` };
}

export default function UserPublicProfilePage() {
  const params = useParams<{ display_name: string }>();
  const displayNameParam = useMemo(() => {
    const v = (params as any)?.display_name;
  ...
  // ✅ default TRUE for this page
  const [includeTradeBlocked, setIncludeTradeBlocked] = useState(true);
  ...
  // 1) Load profile by display_name
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase.from('v_user_profiles').select('*').eq('display_name', displayNameParam).maybeSingle();

      if (error) {
        setBanner({ kind: 'error', msg: `Failed to load user: ${error.message}` });
        setProfile(null);
        setProfileLoading(false);
        return;
      }

      setProfile((data as any) as UserProfileRow | null);
      setProfileLoading(false);
    })();
  }, [displayNameParam]);

  // 2) Load cards for that user
  useEffect(() => {
    (async () => {
  ...
  const x = normalizeXLink(profile?.twitter);
  const discordName = (profile?.discord ?? '').trim() || null;
    ...
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '18px 16px' }}>
      ...
      >
        <div style={{ padding: 14 }}>
          ...
          >
            <div style={{ position: 'absolute', right: 16, top: 16, textAlign: 'right' }}>
              {x.url ? (
                <div style={{ fontSize: 13 }}>
                  <a href={x.url} target="_blank" rel="noreferrer" style={{ textDecoration: 'underline', fontWeight: 900, color: '#111' }}>
                    {x.handle ?? 'X'}
                  </a>
                </div>
              ) : (
                <div style={{ fontSize: 13, color: '#666' }}>X: —</div>
              )}

              <div style={{ marginTop: 6, fontSize: 13, color: discordName ? '#111' : '#666', fontWeight: discordName ? 900 : 700 }}>
                {discordName ? `Discord: ${discordName}` : 'Discord: —'}
              </div>
            </div>

            {profileLoading ? (
              <div style={{ opacity: 0.9 }}>Loading user…</div>
            ) : !profile ? (
              <div style={{ opacity: 0.9 }}>User not found.</div>
            ) : (
              <>
                <div style={{ textAlign: 'center', fontSize: 28, fontWeight: 950, lineHeight: 1.1, paddingRight: 140 }}>{profile.display_name}</div>
                <div style={{ marginTop: 10, textAlign: 'center', color: '#444', whiteSpace: 'pre-wrap', paddingRight: 140 }}>
          ...
          {/* Cards header */}
          <div style={{ marginTop: 14, fontSize: 18, fontWeight: 950 }}>{((profile?.display_name ?? displayNameParam) || 'User') + "'s Cards"}</div>

          {/* Cards grid */}
          {loading ? (
            <div style={{ marginTop: 12, opacity: 0.9 }}>Loading…</div>
          ) : filtered.length === 0 ? (
            <div style={{ marginTop: 12, opacity: 0.9 }}>No cards match your filters.</div>
          ) : (
            <div
              style={{
                marginTop: 12,
                  ...
                  >
                    ...
                    >
                      {playerImg ? (
                        ...
                        <img src={playerImg} alt={r.full_name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      ) : (
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontWeight: 950, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{r.full_name}</div>
                      ...
                      <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        >
                          {flagUrl ? (
                            // eslint-disable-next-line @next/next/no-img-element
                            <img src={flagUrl} alt={team?.short_name ?? 'Nation'} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                          ) : null}
                        </div>
